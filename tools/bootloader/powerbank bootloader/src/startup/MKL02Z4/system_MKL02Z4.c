...
uint32_t GetSystemMCGFLLClock (void)
{
    ...
    // Slow internal ref (IREFS = 0), C2 register cleared to all low frequency settings
    // MCG->C4 DRST_DRS = 01 with DMX32 = 0 = FLL Factor 1280
    // 1280 * Slow Internal Ref (CPU_INT_SLOW_CLK_HZ = 32768hz) = MCGFLLCLK

    return (1280 * CPU_INT_SLOW_CLK_HZ);
    ...
    //     C1[FRDIV] and C2[RANGE0]. See the C4[DMX32] bit description for more details.
    // MCG->C1: CLKS=0,FRDIV=0,IREFS=0,IRCLKEN=1,IREFSTEN=0
    // MCG->C2: LOCRE0=0,RANGE0=0,HGO0=0,EREFS0=1,LP=0,IRCS=0
    // MCG->C4: DMX32=1,DRST_DRS=1
    // DMX32 = 1 and DRST_DRS = 1 == 1464 FLL Factor

    return (1464 * CPU_XTAL_CLK_HZ);
    ...
    //     C4[DMX32] bit description for more details.
    // MCG->C1: CLKS=1,FRDIV=0,IREFS=1,IRCLKEN=1,IREFSTEN=0
    // MCG->C2: LOCRE0=0,RANGE0=0,HGO0=0,EREFS0=0,LP=0,IRCS=1
    // MCG->C4: DMX32=0,DRST_DRS=0 == 640 FLL Factor

    return (640* CPU_INT_FAST_CLK_HZ);
